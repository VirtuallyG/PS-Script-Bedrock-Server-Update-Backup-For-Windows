...
PowerShell script for automatically updating a Bedrock server and backing up the Worlds folder

This script will check for new updates for the Windows bedrock server. If an update is found, it will stop the server, backup the worlds, backup the config files, download, extract, install the update, restore the config files, and restart the server. If there are no new updates, the script will stop the server, back up the worlds, and restart the server. If there have been no changes to the world saves since the last backup, the script will skip the backup.
...
(4) CREATE A POWERSHELL TASK IN WINDOWS TASK SCHEDULER TO RUN PERIODICALLY (WHEN NOBODY IS LIKELY TO BE CONNECTED TO THE SERVER). Google how to set up a PowerShell script to run as a scheduled task


NOTE:

IF YOU ENCOUNTER AN ERROR ABOUT RUNNING SCRIPTS OR NOTICE THAT THE POWERSHELL CONSOLE CLOSES UNEXPECTEDLY, YOU MAY NEED TO ENABLE SCRIPT EXECUTION FIRST.

(1) OPEN AN ELEVATED POWERSHELL (RUN AS ADMINISTRATOR)

(2) RUN THIS COMMAND: `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser`. YOU'LL GET TO THIS PROMPT: "Do you want to change the execution policy?" 

(3) TYPE 'Y' TO SELECT YES, AND THE SCRIPTS SHOULD RUN JUST FINE NOW.
...
(7) CLICK [TASK SCHEDULER LIBRARY] ON THE LEFT SIDE - FIND THE NAME OF YOUR TASK THAT YOU CREATED IN STEP 2 - RIGHT CLICK THE TASK - [RUN]
